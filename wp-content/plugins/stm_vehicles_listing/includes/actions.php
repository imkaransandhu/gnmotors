...
function stm_listings_template_actions() {
	$action = stm_listings_input( 'ajax_action' );
	if ( $action ) {
		...
		switch ( $action ) {
			case 'listings-result':
				...
				if ( stm_is_multilisting() && ! empty( $_GET['posttype'] ) && ! in_array( $_GET['posttype'], array( 'undefined', stm_listings_post_type() ), true ) ) {
...
function stm_listings_ajax_save_user_data() {
	...
	if ( $password_check && ! $demo ) {
		...
		if ( ! empty( $_POST['stm_show_mail'] ) && 'on' === $_POST['stm_show_mail'] ) {
...
function stm_on_publish_pending_post( $post ) {
	...
	if ( stm_me_get_wpcfto_mod( 'send_email_to_user', false ) && in_array( $post->post_type, stm_listings_multi_type( true ), true ) ) {
		$to         = $email->user_email;
